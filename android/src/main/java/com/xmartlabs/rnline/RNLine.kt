...
class RNLine(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
    ...
    private fun login(scopes: List<String>, onlyWebLogin: Boolean, botPromptString: String, promise: Promise) {
        val lineAuthenticationParams = LineAuthenticationParams.Builder()
            .scopes(Scope.convertToScopeList(scopes))
            .apply { botPrompt(LineAuthenticationParams.BotPrompt.valueOf(botPromptString)) }
            .build()

        val lineAuthenticationConfig: LineAuthenticationConfig? = createLineAuthenticationConfig(channelId, onlyWebLogin)
        val activity: Activity = currentActivity ?: return promise.reject("NoActivity", "Current activity is null")

        val loginIntent = when {
            lineAuthenticationConfig != null -> LineLoginApi.getLoginIntent(activity, lineAuthenticationConfig, lineAuthenticationParams)
            onlyWebLogin -> LineLoginApi.getLoginIntentWithoutLineAppAuth(activity, channelId, lineAuthenticationParams)
    ...
    fun getProfile(promise: Promise) {
        uiCoroutineScope.launch {
            ...
            if (!lineApiResponse.isSuccess) {
                promise.reject(lineApiResponse.responseCode.name, lineApiResponse.errorData.message, null)
            } else {
    ...
    fun handleActivityResult(requestCode: Int, resultCode: Int, intent: Intent?) {
        ...
        val result = intent?.let { LineLoginApi.getLoginResultFromIntent(it) }
        if (resultCode != Activity.RESULT_OK || result == null) {
            loginResult?.reject(resultCode.toString(), ERROR_MESSAGE, null)
            return
        }
        ...
        when (result.responseCode) {
            ...
            LineApiResponseCode.CANCEL -> {
                loginResult?.reject(result.responseCode.name, result.errorData.message, null)
            }
            else -> {
                loginResult?.reject(result.responseCode.name, result.errorData.message, null)
            }
    ...
    private fun parseFriendshipStatus(friendshipStatus: LineFriendshipStatus): WritableMap = Arguments.makeNativeMap(
        mapOf("friendFlag" to friendshipStatus.isFriend)
    )
    ...
    private fun parseLoginResult(loginResult: LineLoginResult): WritableMap = Arguments.makeNativeMap(
        mapOf(
            ...
            "scope" to loginResult.lineCredential?.scopes?.let { Scope.join(it) },
    ...
    // Helpers
    private fun createLineAuthenticationConfig(channelId: String, onlyWebLogin: Boolean): LineAuthenticationConfig? {
        return createConfig(channelId, onlyWebLogin)
    }

    private fun createConfig(channelId: String, isLineAppAuthDisabled: Boolean): LineAuthenticationConfig {
